 ...
 *
 * @FieldFormatter(
 *   id = "bootstrap_basic_media_thumbnail_gallery_formatter",
 *   label = @Translation("Bootstrap Basic Media Thumbnail Gallery"),
 *   field_types = {
 *     "entity_reference"
 *   }
 * )
 */
class BootstrapBasicMediaThumbnailGalleryFormatter extends ImageFormatterBase implements ContainerFactoryPluginInterface {
   ...
   * {@inheritdoc}
   */
  public function __construct($plugin_id, $plugin_definition, FieldDefinitionInterface $field_definition, array $settings, $label, $view_mode, array $third_party_settings, EntityStorageInterface $image_style_storage, ThemeManagerInterface $theme_manager, ConfigFactory $configFactory, FileStorageInterface $file_storage) {
    ...
    foreach ($files as $delta => $media) {
      ...
      if ($delta == 0) {
        ...
        $imageGallery['#main'] = [
          ...
          '#cache' => [
            'tags' => Cache::mergeTags($cache_tags, $media->getCacheTags()),
          ],
        ];
      }
